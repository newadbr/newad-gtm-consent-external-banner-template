# Newad Consent Manager - External Banner

## Overview (EN)
Consent template for setups where consent UI is owned by an external banner and GTM updates are event-driven.

External banner mode (enableBanner default = false). Integrates with event `newad_external_accept_all`.
...
2. Search for **Newad Consent Manager - External Banner** in Community Template Gallery.
...
## External Banner Integration (EN)
Use an external HTML listener to push `newad_external_accept_all` to `dataLayer` when user clicks your external consent accept button.

```html
<script>
(function () {
  var SELECTOR = '#root > div.one-button-cookies-popup > div.one-button-cookies-popup-footer-btn';
  ...
  document.addEventListener('click', function (e) {
    ...
    if (e.target === el || el.contains(e.target)) {
      ...
      window.dataLayer.push({ event: 'newad_external_accept_all' });
    }
  }, true);
})();
</script>
...
- Recommended trigger for **Update** tag: Custom Event = `newad_external_accept_all`.


---

## Visao Geral (PT-BR)
Template de consentimento para setups onde a UI de consentimento e externa e os updates no GTM sao disparados por evento.

Modo banner externo (enableBanner default = false). Integra com evento `newad_external_accept_all`.
...
2. Busque **Newad Consent Manager - External Banner** na Community Template Gallery.
...
## Integracao com Banner Externo (PT-BR)
Use um listener HTML externo para enviar `newad_external_accept_all` ao `dataLayer` quando o usuario aceitar no banner externo.
...
- Trigger recomendado para a tag **Update**: Evento Personalizado = `newad_external_accept_all`.
